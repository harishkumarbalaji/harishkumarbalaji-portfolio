  ...
  return (
    <>
      <div className={`social-links ${className} social-links-${size}`}>
        {socialData.links.map((link, index) => {
          ...
          return (
            <a
              ...
              href={
                isResume
                  ? '#'
                  : isDownload
                    ? `${import.meta.env.BASE_URL}${link.url.replace(/^\//, '')}`
                    : link.url
              }
            ...
            >
              <div className="social-icon">{getIconSvg(link.icon)}</div>
            </a>
          );
        })}
      </div>
      ...
      {showResumeModal && resumeUrl && (
        <ResumeModal resumeUrl={resumeUrl} onClose={() => setShowResumeModal(false)} />
      )}
    </>
  );
};
